/**
 * Wordle猜词判定模块
 * 提供猜测结果计算、字母状态推导等纯函数，供 utils 与 renderer 共用
 */
 ...
 * @returns {string} 显示符号
 */
export function getLetterSymbol(letter, status) {
  switch (status) {
    case 'correct':
      return `🟩${letter}`;
    case 'present':
 ...
 */
export function getLetterStatus(guesses, targetWord) {
 ...
 * @returns {Map<string, string>} 字母状态映射
 */
export function getLetterStatusFromResults(guesses, results) {
  const alphabet = 'abcdefghijklmnopqrstuvwxyz';
  const letterStatus = new Map();
  for (const letter of alphabet) letterStatus.set(letter, 'unknown');
